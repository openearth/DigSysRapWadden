...
wl_act_v <- sf::st_read(file.path(datadir, "../", "administratief", "waterlichamen2006.geojson"))
names(wl_act_v) <- tolower(names(wl_act_v))
...
leaflet::leaflet(mijnShape) %>%
  ...
  leaflet::addPolygons(data = sf::st_transform(wl_act_v, 4326), fillColor = "blue", group = "KRW waterlichamen", popup = ~htmltools::htmlEscape(owmnaam)) %>%
