...
if(knitr::is_html_output()) {ggplotly(p, 900, 900) %>%
  config(displayModeBar = F) %>%
  layout(xaxis=list(fixedrange=TRUE)) %>% 
  layout(yaxis=list(fixedrange=TRUE))} else p
...
(ref:plaatvolumeLabel) Plaatvolume in  m^3^ per gebied over de tijd. Het volume is bepaald voor dat deel van de plaat dat tussen -1 m NAP en +1 m NAP  ligt. Het volume is bepaald t.o.v. het referentievlak op -1 m NAP.

```{r plaatvolume, fig.cap = "(ref:plaatvolume)" }
...
(ref:geulvolumeLabel) Geulvolume in miljoen m^3^ per bekken over de tijd. Het volume is bepaald voor dat deel van de geul dat onder de NAP -5 m NAP ligt. 

```{r geulvolume, fig.cap = "(geulvolumeLabel)" }
valid.values.df %>% 
  filter(diepte < -5) %>%
...
```{r plotAllregions, eval=T, fig.height=4, fig.width=7, fig.align='center', fig.cap="Ontwikkeling van de laagwaterlijn, bepaald op een vaste diepte van -2m NAP, voor de gehele Waddenzee minus het Eems estuarium. "}
...
### Ecotopen 2022 - onderliggende kaartlagen

In 2022 wordt een nieuwe ecotopenkaart gemaakt. In deze rapportage laten we de lagen zien die als basis worden gebruikt voor deze ecotopenkaart. 

**Gemiddelde saliniteit**

```{r EcotopenBasis, fig.cap="Basiskaarten voor ecotopenkaart 2022 voor de Waddenzee."}
...
example <- "https://datahuiswadden.openearth.nl/geoserver/ecotopenkaart/wms?service=WMS&version=1.1.0&request=GetMap&layers=ecotopenkaart%3Asaliniteit_200m_gemiddeld&bbox=89900.0%2C539900.0%2C280100.0%2C640100.0&width=768&height=404&srs=EPSG%3A28992&styles=&format=application/openlayers"

wms_ecotopen_2022 <- "https://datahuiswadden.openearth.nl/geoserver/ecotopenkaart/wms?"

# uitgezet omdat de service op dit moment niet in de lucht is.
bwk_client <- WFSClient$new(wms_ecotopen_2022,
serviceVersion = "1.3.0")

#bwk_client$getFeatureTypes(pretty = TRUE)

#Maak een leaflet legend request
url_ecotopen_2022 <- parse_url(wms_ecotopen_2022)
url_ecotopen_2022$query <- list(service = "wms",
                  version = "1.3.0",
                  SLD_VERSION = "1.1.0",
                  request = "GetLegendGraphic",
                  FORMAT = "image/jpeg",
                  LAYER = "ecotopenkaart:saliniteit_200m_gemiddeld",
                  STYLE="")
request_ecotopen_2022 <- build_url(url_ecotopen_2022)
...
leaflet() %>% 
  ...
  addWMSTiles(
    wms_ecotopen_2022,
    layers = "ecotopenkaart:saliniteit_200m_gemiddeld",
    options = WMSTileOptions(format = "image/png", transparent = TRUE),
    group = "gemiddeld saliniteit 2022"
  ) %>%
  addWMSLegend(
    uri = paste0(request_ecotopen_2022),
    position = "bottomright"
  ) %>%
  addLayersControl(
    baseGroups = c("OSM", "Esri.WorldImagery"),
    overlayGroups = c("gemiddelde saliniteit 2022"
                      ),
