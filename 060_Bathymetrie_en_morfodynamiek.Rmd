...
# select some years 
yearsToCalculate <- c(2, 4, 5, 8, 14, 23, 33, 43)     #seq(2,length(mosaiclist), 9)
...
croppedRasters <- lapply(1:nrow(poly), function(x) 
...
valid.values.df  %>%
  ...
  mutate(class = factor(class, levels = c("geul z<-5 m NAP", "subgetijdengebied -5 m NAP<z<-1 m NAP", "intergetijdengebied -1 m NAP<z<1 m NAP", "supragetijdengebied z>1 m NAP"))) %>%
  ...
  facet_grid(naam ~ class, scales = "free") + #free_y
...
```{r plaathoogte, fig.cap = "Plaathoogte per gebied. "}
valid.values.df %>% 
  filter(diepte >= -1 & diepte < 1) %>% #  
  group_by(naam, jaar) %>%
  mutate(oppervlakte = 400/1e6, volume = diepte*400/1e6) %>%
  summarize(plaathoogte = sum(volume)/sum(oppervlakte)) %>%
...
De ontwikkeling van de geuldiepte door de jaren laat voor de westelijke bekkens een toename door de tijd zien (figuur \@ref(fig:geuldiepte)). Verdere analyse is nodig om vast te stellen waar de sprongen in de tijdseries door worden veroorzaakt.
...
```{r plaatvolume, fig.cap = "Plaatvolume in m3 per gebied over de tijd. Het volume is bepaald voor dat deel van de plaat dat boven -1 m tov NAP ligt. Het volume is bepaald tov NAP. " }

valid.values.df %>% 
  filter(diepte >= -1 & diepte < 1) %>%
  mutate(plaathoogte = diepte) %>%    # nu volume t.o.v NAP. Kan ook negatief zijn. 
  group_by(naam, jaar) %>%
...
```{r geulvolume, fig.cap = "Geulvolume in miljoen m3 per bekken over de tijd. Het volume is bepaald voor dat deel van de geul dat onder de NAP -5 m t.o.v. NAP ligt. Het volume is bepaald tov NAP. " }
valid.values.df %>% 
  filter(diepte < -5) %>%
  mutate(volumediepte = -(diepte)) %>%
  group_by(naam, jaar) %>%
  mutate(oppervlakte = 400/1e6, volume = volumediepte*400) %>%
  summarize(geulvolume = sum(volume)/1e6) %>%
  ggplot(aes(jaar, geulvolume)) +
  ggtitle("Geulvolume in miljoen m3.") +
  geom_point() +
  facet_wrap(~ naam, scales = "free")+
  ylab("Geulvomume [miljoen m3] t.o.v. NAP") + xlab("Jaar") 
```
