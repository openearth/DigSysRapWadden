...
```{r gebiedenWaddenzee, fig.cap="Indeling van de Waddenzee in verschillende gebieden voor gebruik in morfologische indicatoren. "}
...
valid.values.df <- map(valid.values, as_tibble) %>%
  ...
  set_names("Marsdiep","Vlie","Borndiep","Eilanderbalg","Pinkegat","Zoutkamperlaag","Schild","Eems-Dollard","Eijerlandse Gat","Lauwers")%>%
  rbindlist(idcol = "naam") %>% 
  mutate(jaar = as.integer(str_replace(jaar, "mosaic_", ""))) %>%
  mutate(naam = factor(naam, levels = c("Marsdiep","Eijerlandse Gat","Vlie","Borndiep","Pinkegat","Zoutkamperlaag","Eilanderbalg","Lauwers","Schild","Eems-Dollard")))

```



```{r hypsometriePerGebied, fig.height=10, fig.width=8, fig.cap="Hypsografische curven voor verschillende gebieden in de Waddenzee en voor een selectie van bathymetrieën. "}
...
valid.values.df %>%
  ...
  mutate(oppervlakte = 400*cum/1e6) %>%
  ...
  facet_wrap(~ naam, scales = "free_x", ncol = 4) +
...
valid.values.df  %>%
  ...
  facet_grid(naam ~ class, scales = "free_y") +
  theme(strip.text.y = element_text(angle = 0)) +
  ggtitle("areaal in km2 per gebied en diepteklasse")
...
De ontwikkeling hiervan door de jaren is 
...
```{r plaatvolume, fig.cap = "Plaatvolume in m2 per gebied over de tijd. Het volume is bepaald voor dat deel van de plaat dat boven -1 m tov NAP ligt. Het volume is bepaald tov NAP. " }

valid.values.df %>% 
  filter(diepte >= -1) %>%
  mutate(plaathoogte = diepte) %>%    # nu volume t.o.v NAP. Kan oook negatief zijn. 
  group_by(naam, jaar) %>%
  mutate(volume = plaathoogte*400) %>% # elk rastersegment is 20x20 m
  summarize(plaatvolume = sum(volume)) %>%
  ggplot(aes(jaar, plaatvolume)) +
  ggtitle("Plaatvolume in m^3.") +
  geom_point() +
  facet_wrap(~ naam)
...
valid.values.df %>% 
  ...
  summarize(geulvolume = sum(volume)) %>%
  ggplot(aes(jaar, geulvolume)) +
  ggtitle("Geulvolume in m3.") +
  geom_point() +
  facet_wrap(~ naam)
```
