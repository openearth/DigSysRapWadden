...
# select some years 
yearsToCalculate <- seq(2,length(mosaiclist), 11)
rasterlist <- mosaiclist[yearsToCalculate]

# read bathymetry and crop to polygons
bathymetries <- stack(file.path(mosaicdir,rasterlist[-1]))
...
group_by(naam, jaar) %>%
  ...
  mutate(oppervlakte = 400*(cum/count)/1000000) %>%
  sample_n(10000) %>%
  ggplot() +
...
```{r arealen, fig.height = 8, fig.width = 8, fig.cap = "Tijdseries met gemeten areaal per gebied en jaar."}
...
valid.values.df %>% 
  filter(diepte >= -1) %>%
  mutate(plaathoogte = diepte - -1) %>%
  group_by(naam, jaar) %>%
  mutate(oppervlakte = 400/1e6, volume = plaathoogte*400) %>%
  summarize(plaatvolume = sum(volume)) %>%
  ggplot(aes(jaar, plaatvolume)) +
  ggtitle("Plaatvolume in m3.") +
  geom_point() +
  facet_wrap(~ naam)
