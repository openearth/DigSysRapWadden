...
## Afbakening, definitie en herkomst {#afbakening}

De Waddenzee is een hoogdynamisch gebied. Geulen en wadplaten zijn continu aan verandering onderhevig. Onder **morfologie** vallen alle indicatoren die te makken hebben met de verandering in bodemligging. De veranderende bodemligging in de Waddenzee is bepalend voor stromingen, golven en waterstanden, maar bijvoorbeeld ook voor scheepvaart (het op diepte houden van vaargeulen), ecologie (habitatbeschikbaarheid). Ook vertelt het iets over de mate waarin de Waddenzee ‘verlandt’  (ondieper wordt ten opzichte van de zeespiegel) of juist ‘verdrinkt’ en geeft informatie over hoe gebieden zich aanpassen aan diverse veranderingen zoals bedijkingen, afsluiting van vroegere zeearmen, bodemdaling en zeespiegelstijging. 

**Begrenzing**

De morfologische indicatoren worden meestal getoond per kombergingsgebied, ook wel bekken genoemd. Figuur \@ref(fig:gebiedenWaddenzee) toont de begrenzing en benaming van de verschillende bekkens volgens Elias (2025). De bekkens Eilanderbalg, Lauwers, Schild en Sparregat worden vaak samen beschouwd onder de naam 'Groninger Wad'. De bekkens kennen elk hun eigen karakteristieke ontwikkeling, gestuurd door een combinatie van hun geometrie en diepteligging en de reactie op ingrepen in het verleden. Ook bestaan er evenwichtsrelaties voor de verhouding tussen geulen en platen afhankelijk van de bekkenoppervlakte.  Bekkens worden begrensd door ‘wantijen’: gebieden achter de eilanden waar de stroomsnelheden het laagst zijn. Hier vormt zich een (vaak slibrijke) hoger gelegen rug tussen de bekkens. Door de morfologische dynamiek van de Waddenzee hebben de wantijen geen vaste positie, ze kunnen bijvoorbeeld van ligging veranderen als gevolg van afsluitingen (zoals de Zuiderzee of de Lauwerszee). 
...
if(refreshpolygon){
  ...
  try(sf::st_write(poly, file.path(datadir, "vakken", "wantijen_2015.geojson")))
}

poly <- sf::st_read(file.path(datadir, "vakken", "wantijen_2015.geojson"), quiet = T)
...
Om iets te kunnen zeggen over de opgetreden veranderingen wordt de Waddenzee verdeeld in verschillende diepteklassen. Zo kunnen we bijvoorbeeld inzoomen op de ontwikkeling van geulen, of juist de droogvallende wadplaten. In grote lijnen komen de diepteklassen overeen met de habitats zoals gedefinieerd binnen Natura2000 en de ecotopenkaart en zijn daarom ook indicatief voor veranderingen in het ecosysteem. Om het morfologische gedrag van het systeem te begrijpen is het echter belangrijk om ontwikkelingen op een langere tijdschaal (decennia) te beschouwen. De onderliggende grote jaarlijkse variatie is wel relevant voor de ecologie en functies als scheepvaart, maar bieden minder zicht op de morfologische veranderingen op lange termijn.  
...
Om vat te krijgen op de ontwikkelingen en trends beschouwen we verschillende indicatoren. Figuur \@ref(fig:infographicMorfologie) laat zien hoe deze indicatoren gecombineerd kunnen worden om ontwikkelingen van een bekken te begrijpen. 

```{r infographicMorfologie, fig.cap="Overzicht van de belangrijkste morfologische indicatoren (volumeveranderingen, hypsometrie, ontwikkeling van arealen) en de wijze waarop deze indicatoren samen informatie geven over de ontwikkeling van een bekken. "}

knitr::include_graphics("images/infographic_indicatoren_morfologie.png")
...
De bodemliggingen voor de gehele Waddenzee in de periode 1985-2020 zijn weergegeven in figuur \@ref(fig:WaddenBathymetrieLeaflet2). 
Zoals uitgebreider beschreven is in de Appendix \@ref(Appmorfologie), zijn bodemkaarten van de Waddenzee een combinatie van lodingen (onder water) en hoogtemetingen (intertidaal en hoger) over de verschillende kombergingsgebieden. Ieder jaar wordt één kombergingsgebied bemeten. Zo wordt elke 6 jaar door Rijkswaterstaat een kaart van de volledige Waddenzee geconstrueerd, op basis van de kaarten van deelgebieden over die periode: de zogenaamde 'Vaklodingscyclus'. Sinds 1985 zijn er 6 volledige cycli geweest, die samen de periode 1985 - 2020 dekken. De 7de cyclus (2021 - 2026) wordt op dit moment bemeten. De serie bodems vanaf 1985 zijn begin 2024 door Deltares opnieuw gecombineerd, aangevuld en gecorrigeerd. Voor de bodems vóór 1985 geldt dat de lodingen minder systematisch plaats vonden en ontsloten werden, waardoor de samengestelde bodems minder duidelijk terug te voeren zijn op een bepaalde periode. 
...
load_from_thredds(file.path(datadir, "RWS", "bathymetrie", "valid_values_df.Rdata"))
...
Om een beter begrip te krijgen van de ontwikkelingen en trends in de Waddenzee kan de bodemligging worden ingedeeld in verschillende diepteklassen, zogenoemde ‘arealen’, zoals bijvoorbeeld geulen of intergetijdengebied. De gehanteerde diepteklassen zijn beschreven in \@ref(afbakening). De oppervlakteverandering van de verschillende diepteklassen is indicatief voor hoe de Waddenzee, en de daarbinnen gelegen bekkens of lokale geulsystemen  zich morfologisch gedragen en hoe de gebieden zich aanpassen aan diverse veranderingen zoals bedijkingen, bodemdaling en zeespiegelstijging.
...
De gemaakte afwegingen en consequenties zijn uitgebreider beschreven in het [rapport (Meijers et al., 2025)](https://publications.deltares.nl/11211571_002_0001.pdf) over de totstandkoming van deze methodiek. 

Figuur \@ref(fig:stackedAreas) toont de oppervlaktes van alle diepteklassen per bekken. In Figuur \@ref(fig:barPlotArealen) worden dezelfde oppervlaktes weergegeven als percentage van het bekkenoppervlak. Arealen berekend vóór 1989 bevatten een grotere onzekerheid door de kwaliteit van bodemkaarten en dekking van waterstandsmetingen uit deze periode. De bekkens in de westelijke Waddenzee (Marsdiep en Vlie) hebben relatief meer subgetijdegebied en geulen, terwijl de bekkens in de oostelijke Waddenzee voor een groter gedeelte uit intergetijdengebied bestaan. 

```{r leesAreaalData}

filenames <- c(
  "Arealen_ZKL_19jr.csv",
  "Arealen_PGAT_19jr.csv",
  "Arealen_AME_19jr.csv",
  "Arealen_VLIE_19jr.csv",
  "Arealen_ELD_19jr.csv",
  "Arealen_MD_19jr.csv",
  "Arealen_GRWAD_19jr.csv",
  "Arealen_ED_19jr.csv",
  "Arealen_Waddenzee_1989_2017_19jr.csv"
)
...
p <- arealen %>%
  ...
  mutate(
    diepteklasse_plotname = factor(diepteklasse_plotname, 
                                   levels = rev(diepteklassen)),
    diepteklasse = factor(diepteklasse, 
                          levels = str_replace(diepteklassen, fixed("\n"), "")),
    basin = factor(basin, levels = basins)
  ) %>%
  ggplot(aes(jaar, area, fill = diepteklasse_plotname)) +

    # ---- CERTAIN years: years >= cutoff ----
  geom_area(
    data = ~subset(.x, jaar >= cutoff_year),
    alpha = 0.8
  ) +

  facet_wrap("basin", scales = "free") +
  scale_fill_manual(values = colors, name = "Diepteklasse") +
  theme_bw() +
  theme(
  ...
  if(plot_as_plotly){
  ggplotly(
    p + ylab("Oppervlakte in km2") + xlab(NULL)
      ) %>% 
      plotly::layout(
        legend = list(
          itemclick = FALSE,
...
-	**Marsdiep**: Dit bekken heeft het kleinste aandeel intergetijdegebied van alle bekkens en ligt voor een relatief groot deel permanent onder water. Er is een toename van subgetijdegebied te zien, die de laatste jaren lijkt af te vlakken.
-	**Eijerlandse Gat**: Het intergetijdegebied is de afgelopen decennia toegenomen, ten koste van met name het subgetijdegebied. Sinds 2005 is deze ontwikkeling afgevlakt en is de verdeling in diepteklassen gelijk gebleven.
-	**Vlie**: Het totale bekkenoppervlak is enkele procenten afgenomen door wantijmigratie (met name uitbreiding van het Marsdiep en Eijerlandse Gat). Tot 1990 nam het intergetijdegebied sterk toe, en namen de arealen subgetijdegebied en ondiepe geul (-3 tot -5 m NAP) af, maar sinds de jaren '90 is deze ontwikkeling gestagneerd.
-	**Borndiep**: Het permanent onder water liggende areaal (geulen en subgetijdegebied) neemt licht af, terwijl het kwelderoppervlak enkele procenten uitbreidt, met name door groei van de vastelandskwelders. 
-	**Pinkegat en Zoutkamperlaag** (vormen samen het Friesche Zeegat): In het Pinkegat is het oppervlak intergetijdegebied sterk toegenomen voor 1990, en nam het subgetijdegebied af. Sinds 1990 is de verdeling van diepteklassen voor beide bekkens relatief gelijk gebleven. 
-	**Groninger Wad**: Het aandeel intergetijdegebied is licht afgenomen, met name door een uitbreiding van subgetijdegebied.
-	**Eems-Dollard:** hier is het intergetijdegebied toegenomen. Dit is ten koste gegaan van met name het subgetijdegebied en de ondiepe geulen. Deze ontwikkeling kan worden verklaard door grootschalige sedimentatie op met name het Eemshornwad en Hond Paap, waar subgetijdegebied is veranderd in intergetijdegebied. 

In de Waddenzee als geheel (getoond van 1989 tot 2017, de periode waarvoor gebiedsdekkende kaarten beschikbaar zijn voor de gehele Waddenzee) is het areaal intergetijdegebied met enkele procentpunten toegenomen en het subgetijdegebied licht afgenomen. 

```{r barPlotArealen, fig.width = 8, fig.height = 8, fig.cap = "Relatieve areaalontwikkeling van bekkens in de Waddenzee. Let op: data van voor 1989 is weergegeven met transparante staven (aangegeven als 'False' in legenda). Arealen berekend voor deze jaren bevatten een grotere onzekerheid door lagere kwaliteit van historische bodemkaarten en beschikbaarheid van waterstanden."}

p <- arealen %>%
  filter(diepteklasse != "Totaal") %>%
  mutate(
    # Step 1: ensure jaar is numeric
    jaar = as.numeric(jaar),

    # Step 2: compute transparency flag BEFORE converting to factor
    recent = jaar >= 1989,

    # Step 3: convert jaar to ordered factor for plotting
    jaar = factor(jaar, levels = sort(unique(jaar))),

    # Your existing ordering logic
    diepteklasse_plotname = factor(diepteklasse_plotname, levels = diepteklassen),
    diepteklasse = factor(diepteklasse, levels = str_replace(diepteklassen, fixed("\n"), "")),
    basin = factor(basin, levels = basins)
  ) %>%
  ggplot(aes(diepteklasse_plotname, percentage)) +
  geom_col(
    aes(fill = jaar, alpha = recent),
...
load_from_thredds(file.path(datadir, "RWS", "bathymetrie", "hoogtediepte_bekkens.Rdata"))
...
melt_matrix <- function(mat) {
  data.frame(
    Var1  = rep(seq_len(nrow(mat)), times = ncol(mat)),
    Var2  = rep(seq_len(ncol(mat)), each  = nrow(mat)),
    value = as.vector(mat)
  )
}
...
sedimentatlas <- melt_matrix(RNetCDF::var.get.nc(con, "phi")) %>%
  mutate(
    diam = diameters[Var1],
    lng  = lons[Var2],
    lat  = lats[Var2]
