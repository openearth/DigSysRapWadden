...
```{r WaddenBathymetrieLeaflet2, fig.cap = "Bathymetrie van de Waddenzee, samengesteld uit lodingen tussen 1926 en 2021. De kaart laat bodemligging zien voor het gekozen interval. Het interval kan verschoven worden door tussen de punten te klikken en slepen, en ook kan het interval breder of smaller gemaakt worden door de punten te verslepen. "}

knitr::include_url("https://deltares.shinyapps.io/bathymetrywadden/", height = "600px")

```
Zoals uitgebreider beschreven is in de Appendix \@ref(Appmorfologie), zijn de verschillende bodemkaarten van de Waddenzee een samenslag van lodingen (onder water) en hoogtemetingen (intertidaal en hoger) over de verschillende kombergingsgebieden waarvan er jaarlijks één bemeten wordt. Zo is elke 6 jaar door Rijkswaterstaat een kaart van de volledige Waddenzee geconstrueerd, op basis van de kaarten van deelgebieden over die periode. De eerste volledige kaart dateert van 1991, met data over de periode 1985-1991. Let wel, deze kennen missende punten en vooral de kwelders en ondiepe gebieden langs de eilandkusten zijn niet volledig. De serie bodems vanaf 1985 zijn begin 2024 door Deltares opnieuw gecombineerd, aangevuld en gecorrigeerd. Voor de bodems vóór 1985 geldt dat de lodingen minder systematisch plaats vonden en ontsloten werden, waardoor de samengestelde bodems minder duidelijk terug te voeren zijn op een bepaalde periode.  


## Netto erosie en sedimentatie

Figuur \@ref(fig:diffmaps) geeft de netto erosie en sedimentatie aan gebaseerd op dezelfde bathymetriegegevens als figuur \@ref(fig:WaddenBathymetrieLeaflet2). Er is ook een [schermvullende versie](https://deltares.shinyapps.io/miniDiffMaps/) beschikbaar. 

```{r diffmaps, fig.cap= "Interactieve verschilkaarten voor bodemligging. Verschillende verschilperioden kunnen gekozen worden. De kleurenschaal is aanpasbaar. Verschillen buiten de gekozen kleurenschaal worden niet weergegeven (transparant). " }
...
Voor deze rapportage zijn gebiedsdekkende kaarten gegenereerd voor de Waddenzee op basis van het gemiddelde jaarlijkse hoogwater en laagwater gemeten op de verschillende stations. Op basis hiervan kan aan de hand van de bathymetrie worden vastgesteld welk deel van de Waddenzee zich onder de laagwaterlijn bevindt (subgetijdengebied) en welk deel hoger ligt dan de hoogwaterlijn (supragetijdengebied). Het areaal dat tussen het gemiddelde hoogwater en laag water ligt is het intergetijdengebien. Areaal dat zich meer dan 2 meter onder de laagwaterlijn ligt wordt aangeduid als geul. De gebiedsdekkende kaarten zijn vooralsnog gegenereerd op basis van 'inverse-distance weighted (idw)' interpolatie (idp = 3).
...
#Niet voor ieder jaar is data aanwezig, dus we gaan extrapoleren voor jaren die er niet zijn
...
# We willen output voor de volgende jaren 
doeljaren <- c("1927","1949","1971","1975","1991","2003","2009","2015", "2019")

# We maken nu twee data frames. Ws bevat waterstanden op basis van de lineaire lijn 
# over alle gemeten jaren. Extrema bevat de gemeten waardes, behalve voor de missende data, 
# hiervoor worden de geëxtrapoleerde waterstanden uit ws gebruikt.
...
# Nu voegen we de coordinaten van de verschillende locaties toe
...
# We willen een lang data frame format 
...
# Interpolatie voorbereiden 

# Resolutiefactor
fac = 10 #De bathymetrieën hebben een resolutie van 20x20 m, deze factor bepaalt waarmee deze vermenigvuldigd wordt, belangrijk voor looptijd

# We gebruiken het domein en crs van één van de bathymetrische .tif bestanden die beschikbaar is
# Resolutie in dit geval 20 *fac meter
bathymetry1927 <- raster::raster(file.path(mosaicdir, mosaiclist[1]))
#bathymetry1927_lowres <- aggregate(bathymetry1927, fact=10)
r_lowres <- raster(extent(bathymetry1927), crs = crs(bathymetry1927), resolution = c(20*fac,20*fac)) 

# We maken G objecten die vervolgens geinterpoleerd kunnen worden
# De package gstat wordt hiervoor gebruikt
spdfs <- list()
spdfs_trans <- list()
gobjs <- list()

unique_indicators <- unique(extrema_full_l$ind) #iedere combinatie jaar+H/L-water
...
# We maken eerst een lijst met de doeljaren, bathymetrieën, hoogwaterlijnen en laagwaterlijnen 
# De bathymetriën laden we in
bathy_lowres <- c()
...
lagen_lowres <- list(jaar = doeljaren, bathymetry = bathy_lowres, laagwater = watervlakken_lowres[10:18], hoogwater = watervlakken_lowres[1:9])

# We voegen voor ieder jaar de areaalvlakken toe
...
for (i in 1:length(doeljaren)){
  lagen_lowres$subtidal[[i]] <- reclassify(lagen_lowres$laagwater[[i]] - lagen_lowres$bathymetry[[i]], matrix)
  #lagen_lowres$geul[[i]] <- reclassify(lagen_lowres$bathymetry[[i]]+5, matrix_inv)
  lagen_lowres$geul[[i]] <- reclassify((lagen_lowres$laagwater[[i]]-2)-lagen_lowres$bathymetry[[i]],matrix)
  lagen_lowres$supratidal[[i]] <- reclassify(lagen_lowres$bathymetry[[i]]-lagen_lowres$hoogwater[[i]], matrix)
  lagen_lowres$intertidal[[i]] <- reclassify(reclassify(lagen_lowres$laagwater[[i]]-lagen_lowres$bathymetry[[i]], matrix_inv)-
                                        reclassify(lagen_lowres$bathymetry[[i]]-lagen_lowres$hoogwater[[i]], matrix_inv),rcl =cbind(from = c(-Inf, -.01, .01),to = c(-.01, .01, Inf), become = c(0,1,0)))
}


save(lagen_lowres, file = file.path(datadir, "RWS", "bathymetrie", "lagen.Rdata"))
```

```{r arealen, fig.height = 12, fig.width = 12, fig.cap = "Tijdseries met gemeten areaal in km^2^ per gebied en diepteklasse."}

load(file = file.path(datadir, "RWS", "bathymetrie", "lagen.Rdata"))

# Voor het croppen moeten we de sf omzetten naar sp
poly_sp <- as(poly, 'Spatial')

bekkens <- poly_sp$name
hoogteklassen <- c('geul', 'subtidal', 'intertidal', 'supratidal')
bekkens.df <- data.frame(jaar = integer(), naam = integer(), oppervlakte = numeric(), areaal = character())

for(hoogteklasse in hoogteklassen){
  for(i in 1:length(lagen_lowres[[hoogteklasse]])) {
    for(j in 1:length(poly_sp)) {
      masked <- mask(crop(lagen_lowres[[hoogteklasse]][[i]], extent(poly_sp[j,])), poly_sp[j,])
      oppervlakte <- sum(masked@data@values, na.rm=T) *200 *200 /1000000
      bekkens.df <- rbind(bekkens.df, data.frame(jaar = doeljaren[i], naam = bekkens[j], oppervlakte = oppervlakte, areaal=hoogteklasse))
    }
  }
}

if (remove_seventies_easternWadden){
  bekkens.df <- bekkens.df %>% 
    filter(!(naam %in% c("Lauwers", "Eilanderbalg", "Schild") & jaar < 1972))
}

bekkens.df$naam <- factor(bekkens.df$naam, levels = namen_west_oost)

bekkens.df <- bekkens.df %>%
  mutate(areaal = ifelse(areaal == 'subtidal', 'subgetijdengebied', areaal)) %>% 
  mutate(areaal = ifelse(areaal == 'intertidal', 'intergetijdengebied', areaal)) %>% 
  mutate(areaal = ifelse(areaal == 'supratidal', 'supragetijdengebied', areaal))

bekkens.df$areaal <- factor(bekkens.df$areaal, levels = c('geul', 'subgetijdengebied', 'intergetijdengebied', 'supragetijdengebied'))

ggplot(aes(jaar, oppervlakte), data= bekkens.df %>% 
         mutate(oppervlakte = as.numeric(oppervlakte)) %>% 
    ...
    facet_wrap(~ interaction(areaal, naam), ncol = 4, scales = "free") +
...
```{r arealenOld, fig.height = 12, fig.width = 12, fig.cap = "Tijdseries met gemeten areaal in km^2^ per gebied en diepteklasse.", include=TRUE}
...
deelgebieden_sp <- as(deelgebieden_sf$geometry, 'Spatial')

deelgebieden.df <- data.frame(jaar = integer(), gebied = integer(), oppervlakte = numeric(), areaal = character())

for(hoogteklasse in hoogteklassen){
  for(i in 1:length(lagen_lowres[[hoogteklasse]])) {
    for(j in 1:length(deelgebieden_sp)) {
      masked <- mask(crop(lagen_lowres[[hoogteklasse]][[i]], extent(deelgebieden_sp[j,])), deelgebieden_sp[j,])
      oppervlakte <- sum(masked@data@values, na.rm=T) *200 *200 /1000000
      deelgebieden.df <- rbind(deelgebieden.df, data.frame(jaar = doeljaren[i], naam = deelgebieden[j], oppervlakte = oppervlakte, areaal=hoogteklasse))
    }
  }
}

deelgebieden.df <- deelgebieden.df %>%
  mutate(areaal = ifelse(areaal == 'subtidal', 'subgetijdengebied', areaal)) %>% 
  mutate(areaal = ifelse(areaal == 'intertidal', 'intergetijdengebied', areaal)) %>% 
  mutate(areaal = ifelse(areaal == 'supratidal', 'supragetijdengebied', areaal))

ggplot(aes(jaar, oppervlakte), data= deelgebieden.df %>% 
         mutate(oppervlakte = as.numeric(oppervlakte)) %>% 
         mutate(jaar = as.numeric(jaar))) +
    geom_line(aes()) +
    geom_point(aes(), shape = 21, fill = "white", size = 3) +
...
```{r arealenDeelgebiedenOld, fig.height = 12, fig.width = 12, fig.cap = "Tijdseries met gemeten areaal in km^2^ per deelgebied en diepteklasse.", include=TRUE}
...
```{r plaathoogteDeelgebieden, fig.cap = "Gemiddelde plaathoogte per deelgebied. "}
...
```{r geuldiepte, fig.cap="Geuldiepte door de jaren in de verschillende bekkens. Let op de verschillende verticale assen. "}
...
```{r geuldiepteDeelgebieden, fig.cap="Geuldiepte door de jaren in de verschillende bekkens. Let op de verschillende verticale assen. "}
...
Bovenstaande wordt beoordeeld en geduid in de kombergingsrapportages aan de hand van een aantal (bijna 200 voor de gehele Waddenzee) geuldoorsnedes (profielen) op vaste locaties, wat een vergelijking tussen verschillende jaren mogelijk maakt. Klikken op een profiellijn in Figuur \@ref(fig:geul-kaart) geeft een figuur met de geuldoorsnedes voor de vaklodingen tussen 1927 en nu. De data na 1991 zijn gecombineerde bathymetriën van de vaklodingen die per kombergingsgebied per jaar plaatsvinden, de data daarvoor zijn ook combinaties van bathymetrieën van meerdere jaren maar de precieze herkomst niet altijd duidelijk. Ze refereren dus naar een geuldoorsnedes per periode. De data van de gecombineerde bathymetriën is ook te vinden op Datahuis Wadden. 
