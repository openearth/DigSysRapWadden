...
remove_seventies_easternWadden <- function(df) {
  df <- df %>% filter(!(naam %in% c("Lauwers", "Eilanderbalg", "Schild") & jaar < 1972))
}
...
remove_seventies_easternWadden(valid.values.df)
...
```{r arealenOld, fig.height = 12, fig.width = 12, fig.cap = "Tijdseries met gemeten areaal in km^2^ per gebied en diepteklasse.", include=FALSE}
...
```{r arealenDeelgebiedenOld, fig.height = 12, fig.width = 12, fig.cap = "Tijdseries met gemeten areaal in km^2^ per deelgebied en diepteklasse.", include=FALSE}
...
```{r plaathoogteOld, fig.cap = "Gemiddelde plaathoogte per bekken. ", include=FALSE}
valid.values.df %>% 
...
```{r plaathoogteDeelgebiedenOld, fig.cap = "Gemiddelde plaathoogte per bekken. ", include=FALSE}
...
```{r geuldiepteOld, fig.cap="Geuldiepte door de jaren in de verschillende gebieden. Let op de verschillende verticale assen. ", include=FALSE}
...
```{r geuldiepteDeelgebiedenOld, fig.cap="Geuldiepte door de jaren in de verschillende deelgebieden. Let op de verschillende verticale assen. ", include=FALSE}
...
```{r plaatvolumeOld, fig.cap = "Plaatvolume in  m^3^ per gebied over de tijd. Het volume is bepaald voor dat deel van de bodem dat tussen -1 m NAP en +1 m NAP ligt. ", include=FALSE }
...
```{r plaatvolumeDeelgebiedenOld, fig.cap = "Plaatvolume in  m^3^ per deelgebied over de tijd. Het volume is bepaald voor dat deel van de bodem dat tussen -1 m NAP en +1 m NAP ligt. ", include=FALSE }
...
```{r geulvolumeOld, fig.cap = "Geulvolume in miljoen m^3^ per bekken over de tijd. Het volume is bepaald voor dat deel van het bekken dat onder de NAP -5 m NAP ligt. ", include =FALSE }
valid.values.df %>% 
  filter(diepte < -5) %>%
...
```{r geulvolumeDeelgebiedenOld, fig.cap = "Geulvolume in miljoen m^3^ per deelgebied over de tijd. Het volume is bepaald voor dat deel van het deelgebied dat onder de NAP -5 m NAP ligt. " , include = FALSE}
valid.values.Marsdiep.df %>% 
  filter(diepte < -5) %>%
  mutate(volumediepte = -(diepte+5)) %>%
  group_by(naam, jaar) %>%
