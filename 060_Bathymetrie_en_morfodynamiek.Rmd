...
valid.values.df <- map(valid.values, as_tibble) %>%
  ...
  set_names("Marsdiep","Vlie","Borndiep","Eilanderbalg","Pinkegat","Zoutkamperlaag","Schild","Eierlandse Gat","Lauwers")%>%
  rbindlist(idcol = "naam") %>%
  mutate(jaar = as.integer(str_replace(jaar, "mosaic_", ""))) %>%
  mutate(naam = factor(naam, levels = c("Marsdiep","Eierlandse Gat","Vlie","Borndiep","Pinkegat","Zoutkamperlaag","Eilanderbalg","Lauwers","Schild")))
...
De ontwikkeling van de geuldiepte door de jaren laat voor de westelijke bekkens (Marsdiep, Eierlandse Gat, Vlie) een toename door de tijd zien (figuur \@ref(fig:geuldiepte)). Verdere analyse is nodig om vast te stellen waar de sprongen in de tijdseries in sommige bekkens door worden veroorzaakt.
...
Het geulvolume is in de westelijke bekkens (Marsdiep, Eierlandse Gat, Vlie) toegenomen, terwijl deze in het Friesche Zeegat (Pinkegat+Zoutkamperlaag) is afgenomen (figuur \@ref(fig:geulvolume)). De scherpe afname van het volume van Zouterkamperlaag begin jaren zeventig komt door de sluiting van de Lauwerszee in 1968.
...
contour.df %>%
  mutate(`laagwaterlijnlengte (km)` = laagwaterlijnlengte_m/1000) %>%
    mutate(gebied = factor(gebied, levels = c("Marsdiep","Eierlandse Gat","Vlie","Borndiep","Pinkegat","Zoutkamperlaag","Eilanderbalg","Lauwers","Schild"))) %>%
