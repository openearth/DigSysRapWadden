...
contours_allregions <- lapply(as.list(bathymetries), function(x)
  laagwaterlijn = st_as_sf(rasterToContour(x, level = -1))
)
names(contours_allregions) <- doeljaren

save(contours_allregions, file = file.path(datadir, "RWS", "bathymetrie", "1mNAPcontourlijnen.Rdata"))
...
contours <- lapply(croppedRasters, function(x) 
  tibble(
    ...
    laagwaterlijnlengte = unlist(lapply(as.list(x), function(y)
      st_length(st_as_sf(rasterToContour(y, level = -1)))
    ))
  )
)
...
```{r, eval=F}
load(file = file.path(datadir, "RWS", "bathymetrie", "1mNAPcontourlijnen.Rdata"))
...
```{r lengteLaagwaterLijn, fig.cap="Ontwikkeling (tijdreeks) van de lengte van de laagwaterlijn, geschat uit de contour op -1 m NAP. " }
