...
Sommige indicatoren zijn gekoppeld aan (al dan niet wettelijke) doelstellingen, vanuit verschillende kaders. Via de koppelingen in de tabel hieronder, is het mogelijk om snel de juiste selectie indicatoren te vinden, behorend bij een bepaald kader.
...
kadertabel <- readxl::read_excel(file.path(datadir, "RWS/Indicatoren behorend bij kaders_v0.1.xlsx"), skip = 3) %>% replace_na(list(Kader = "", Indicator = "", Snelkoppeling = "")) %>%
  mutate(Snelkoppeling = paste0("[", Indicator, "]", "(", Snelkoppeling, ")"))
