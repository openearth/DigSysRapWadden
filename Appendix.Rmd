...
```{r}
download_link(
  link = file.path(projectDataPath, mijnGebied, "RWS", "standard", paste0("extremaHLLL", "yearlyaverages.csv")),
  button_label = "Download jaargemiddelde extrema als .csv",
  button_type = "primary",
  has_icon = TRUE,
  icon = "fa fa-save",
  self_contained = FALSE
...
De getijcomponenten zijn berekend op basis van de waterstationsmeetseries van Deltares.
...
Getijcomponenten zijn berekend per station en jaar, en kunnen gedownloadt worden vanaf [dit adres](https://watersysteemdata.deltares.nl/thredds/catalog/watersysteemdata/Wadden/ddl/calculated/TA_filtersurge/catalog.html).
...
De astronomische waterstand is berekend per station en jaar, en kan gedownloadt worden vanaf [dit adres](https://watersysteemdata.deltares.nl/thredds/catalog/watersysteemdata/Wadden/ddl/calculated/waterstand_berekend_m/catalog.html).

## Golven {#Appgolfhoogte}

### Brondata

Significante golfhoogte, golfperiode (Tm02) en golfrichting worden eveneens rechtstreeks ingelezen uit de Data DistributieLaag. De ingelezen set die gebruikt is voor deze rapportage is beschikbaar via onderstaande download link. 
...
```{r}
download_link(
  file.path("http://watersysteemdata.deltares.nl/thredds/fileServer/watersysteemdata/Wadden", "RWS", "standard", "WQ_TS_trendstations_allyears.csv"),
...
Voor de opdeling in kombergingen is een indeling van "tidal basins" gebruikt. De polygonen van de vakken zijn te downloaden van de [Wageningen Marine Research Geoserver]("https://opengeodata.wmr.wur.nl/geoserver/WS3shp/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=WS3shp%3Aws3_tidalbasins&outputFormat=application/json")
...
De versie van het rapport staat op het titelblad. 
...
De code wordt bewaard op SVN onder versiebeheer (https://repos.deltares.nl/repos/DigSysRapWadden/). Om bij te dragen is het nodig om geregistreerd te zijn. Mensen buiten Deltares hebben bovendien een OpenEarth account nodig. 
