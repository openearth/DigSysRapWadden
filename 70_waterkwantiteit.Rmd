...
df.extrema.jaar <- df.extrema %>%
  ...
  summarise(jaargemiddelde = mean(h, na.rm = T)) %T>%
  write_csv(pad)

```

```{r Hoogwaters, fig.height=6, fig.width=6, fig.cap='Jaargemiddeld hoogwater (GHW) per station (punten). De doorgetrokken lijn is een lineaire regressie door de alle data rekening houdend met de 18,6-jarige cyclus; de grijze band is het 95% predictie-interval van het regressiemodel.'}
...
p <- df.extrema %>%
  ...
  summarise(jaargemiddelde = mean(h, na.rm = T)) %T>%
  write_csv(file.path(projectDataPath, mijnGebied, "RWS", "standard", paste0("extremaHLLL", "yearlyaverages.csv"))) %>%
    ggplot() +
  geom_point(aes(x = jaar, y = jaargemiddelde, color = locatie.naam), size = 1) +
  geom_smooth(
    aes(jaar, jaargemiddelde, color = locatie.naam), 
    method = "lm", 
...
```{r Laagwaters, fig.height=8, fig.width=8, fig.cap="Jaargemiddeld laagwater (GLW) per station (punten). De doorgetrokken lijn is een lineaire regressie door de alle data rekening houdend met de 18,6-jarige cyclus; de grijze band is het 95% predictie-interval van het regressiemodel."}
...
```{r getijslag, fig.height=8, fig.width=8, fig.cap= 'Jaargemiddelde getijslag per station (punten). De doorgetrokken lijn is een lineaire regressie door de alle data rekening houdend met de 18,6-jarige cyclus; de grijze band is het 95% predictie-interval van het regressiemodel.'}
...
```{r ecotopenStroming fig.cap="Kaart van het gedurende 95% van de tijd overschreden, dus vrijwel maximale, stroomsnelheid afkomstig uit modelberekeningen gebruikt voor de Ecotopenkaart."}
...
Hieronder zijn de orbitaalsnelheden gepresenteerd. Dit zijn modelbewerkingen die ook gebruikt zijn in het maken van de huidige ecotopenkaarten. De orbitaalsnelheden zijn in detail te bekijken zijn in de Waddenviewer van [Datahuis Wadden](https://viewer.openearth.nl/wadden-viewer/?layers=154102470&layerNames=Orbitaalsnelheid&folders=93790295,154102245). Voor details van het model en de berekeningswijze wordt verwezen naar Van Weerdenburg (2022) [download](https://publicwiki.deltares.nl/download/attachments/137135283/11208040-011-ZKS-0001_v1.0-Modelparameters%20Ecotopenkaart%20Waddenzee%202022%20%28definitief%29.pdf?version=1&modificationDate=1670514304038&api=v2).

```{r ecotopenkaartOrbitaal}
...
```{r kleineZoetwaterDataInlezen}

#hhnkDat <- data.table::fread(hollandsnoorderkwartierpath, na.strings = "-999")

hhnkLoc = "https://datahuiswadden.openearth.nl/geoserver/zoetwaterafvoeren/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=zoetwaterafvoeren%3Ahhnk&outputFormat=csv&cql_filter=%22locatie.naam%22="

hhnkLocations <- hhnkLocations %>%
         mutate(quoted_items = str_glue("'{locatie.naam}'")) %>% # Add single quotes around the items
         mutate(quoted_items = str_replace_all(quoted_items," ", "%20"),
         final_url = paste0(hhnkLoc, quoted_items))  # Concatenate the base URL with the quoted items

hhnkData <- map_dfr(hhnkLocations$final_url, read.csv, na.strings = "-999")

frieslandPath <- "https://datahuiswadden.openearth.nl/geoserver/zoetwaterafvoeren/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=zoetwaterafvoeren%3Afriesland&outputFormat=csv"
friesDat <- data.table::fread(frieslandPath)

NoorderzijlPath <- "https://datahuiswadden.openearth.nl/geoserver/zoetwaterafvoeren/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=zoetwaterafvoeren%3Anoorderzijlvest&outputFormat=csv"
NoorderzijlDat <- data.table::fread(NoorderzijlPath)

hunzeLoc<- "https://datahuiswadden.openearth.nl/geoserver/zoetwaterafvoeren/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=zoetwaterafvoeren%3Ahunzeenaas&outputFormat=csv&cql_filter=%22locatie.naam%22="

hunzeLocations <- hunzeLocations %>%
  mutate(quoted_items = str_glue("'{locatie.naam}'"),  # Add single quotes around the items
         final_url = paste0(hunzeLoc, quoted_items))  # Concatenate the base URL with the quoted items

hunzeData <- map_dfr(hunzeLocations$final_url, read.csv)
```

***Hollands Noorderkwartier***

```{r hhnkAfvoerTijdserie,eval=FALSE, fig.cap="Tijdseries van zoetwaterafvoeren Hoogheemraadschap Hollands Noorderkwartier. "}
#that cell is still turned off the final graph is not correct

table(hhnkData$kwaliteitsoordeel.code)
# wat doen met onbetrouwbare waarden? voor nu alleen een filter op betrouwbaar

hhnkData2<-hhnkData %>%
  filter(kwaliteitsoordeel.code == "betrouwbaar") %>%
  # make daily average
  mutate(datum = as.POSIXct(datum, format="%d-%m-%Y %I:%M")) %>%
  mutate(
    dag = day(as_date(datum)),
    jaar = year(as_date(datum)), 
    maand = month(as_date(datum))
  ) %>%
  group_by(locatie.naam, dag, grootheid.code, eenheid.code, kwaliteitsoordeel.code, jaar, maand)  %>%
  summarize(daggemiddelde = mean(numeriekewaarde, na.rm = F), count = n()) #sunmmarizing the column numeriekewaarde

parlabel = paste(unique(hhnkDat$grootheid.code), unique(hhnkDat$eenheid.code))
xlabel = "jaar"

ggplot() +
  geom_point(data = hhnkData2, aes(as.Date(lubridate::ymd(paste(jaar, maand, dag))), daggemiddelde, color = "daggemiddelde"), alpha = 1, size = 0.65) + #
  geom_point(data = hhnkData2 %>%
               group_by(locatie.naam, jaar, maand) %>% summarize(maandgemiddelde = mean(daggemiddelde), n = n()),
             aes(as.Date(lubridate::ymd(paste(jaar, maand, "15"))), maandgemiddelde, color = "maandgemiddelde"), size = 1
  )  +
  geom_line(data = hhnkData2 %>%
...
parlabel = paste(unique(friesDat$grootheid.code), unique(friesDat$eenheid.code))
xlabel = "jaar"

ggplot() +
  geom_point(data = friesDat2, aes(datum, numeriekewaarde, color = "daggemiddelde"), alpha = 1, size = 0.65) + #
  geom_point(data = friesDat2 %>%
               group_by(locatie.naam, jaar, maand) %>% summarize(maandgemiddelde = mean(numeriekewaarde), n = n()),
             aes(as.Date(lubridate::ymd(paste(jaar, maand, "15"))), maandgemiddelde, color = "maandgemiddelde"), size = 1
  ) +
  geom_line(data = friesDat2 %>%
...
```{r hunzeAfvoerTijdserie,eval=FALSE, fig.cap="Tijdseries van zoetwaterafvoeren waterschap Hunze en Aa's. "}
...
hunzeData2 <-hunzeData %>%
  ...
  summarize(daggemiddelde = mean(numeriekewaarde, na.rm = F), count = n()) #


ggplot() +
  geom_point(data = hunzeData2, aes(as.Date(lubridate::ymd(paste(jaar, maand, dag))), daggemiddelde, color = "daggemiddelde"), alpha = 1, size = 0.65) +
   geom_point(data = hunzeData2 %>%
               group_by(locatie.naam, jaar, maand) %>% summarize(maandgemiddelde = mean(daggemiddelde), n = n()),
             aes(as.Date(lubridate::ymd(paste(jaar, maand, "15"))), maandgemiddelde, color = "maandgemiddelde"), size = 1
  )  +
  geom_line(data = hunzeData2 %>%
              group_by(locatie.naam, jaar, maand) %>% summarize(maandgemiddelde = mean(daggemiddelde), n = n()) %>% ungroup() %>%
              group_by(locatie.naam, jaar) %>% summarize(jaargemiddelde = mean(maandgemiddelde), `n/jaar` = sum(n)) %>% ungroup() %>% filter(`n/jaar` > 10),
            aes(as.Date(lubridate::ymd(paste(jaar, "07", "01"))), jaargemiddelde, color = "jaargemiddelde"), size = 1.5, alpha = 0.8
  ) +
