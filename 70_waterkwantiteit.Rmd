...
RWSlocations <- afvoer.df %>% distinct(loccod, locoms, gbdoms, loc_xcrdgs, loc_ycrdgs) %>%
  mutate(loc_xcrdgs = loc_xcrdgs/100, loc_ycrdgs = loc_ycrdgs/100) %>%
  rename(locatie.naam = locoms, locatie.code = loccod, gebied = gbdoms) %>%
...
ggplot() +
  geom_point(data = friesDat2, aes(datum, numeriekewaarde, color = "metingen"), alpha = 1, size = 0.65) + #
...
ggplot() +
  geom_point(data = NoorderzijlDat2, aes(datum, numeriekewaarde, color = "metingen"), alpha = 1, size = 0.65) + #
