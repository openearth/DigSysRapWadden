...
gemGetijslag <- rbindlist(map(extrema, function(x) x$HL), idcol = "locatie.naam") %>%
  ...
  group_by(locatie.naam, `gemiddelde getijslag in cm` = mean(`getijslag in cm`, na.rm = T)) %>%
  ungroup() %>%
...
leaflet() %>%
  addTiles() %>%
  addCircleMarkers(radius = 7, label = ~paste(locatie.naam, ", gemiddelde getijslag =", `gemiddelde getijslag in cm`, "cm"), color = ~ pal(`gemiddelde getijslag in cm`)) %>%
