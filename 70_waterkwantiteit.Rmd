...
Dit hoofdstuk beschrijft de hydrodynamiek - ofwel de waterbeweging - van de Waddenzee. De volgende parameters worden beschreven:

-	**Waterstanden:** Waterstandsverschillen als gevolg van het getij drijven stromingen aan die voor grootschalig transport van sediment en nutriënten zorgen. Waterstanden bepalen ook de overstromings- of droogvalduur van wadplaten en kwelders, en daarmee welke soorten er wel of niet voor kunnen komen. Naast een aantal relevante waterstandsparameters (Gemiddeld hoog- en laagwater, getijslag, getij-asymmetrie en de belangrijkste getijcomponenten) wordt ook de zeespiegsltijging beschreven.
-	**Stroming:** Stroming wordt gestuurd door waterstandsverschillen door het getij, en speelt een belangrijke rol in zowel de morfologie als ecologie van de Waddenzee.
-	**Golven:** Golven beïnvloeden de erosie en depositie van sediment, de concentraties van zwevende stof in het water (troebelheid) en verstoring van biota. Naast golfhoogte worden ook orbitaalsnelheid (snelheid op de bodem die wordt veroorzaakt door golven) en golfperiode gegeven.
-	**Zoetwaterafvoeren:** Vanuit verschillende locaties wordt zoet water op de Waddenzee geloosd (spuien). De grootste afvoerpunten zitten bij de Afsluitdijk (Den Oever en Kornwerderzand) en bij het Lauwersmeer. Dit heeft effect op het zoutgehalte in de Waddenzee. Het zoutgehalte heeft een directe invloed op de ecologie, omdat sommige soorten alleen zout water tolereren en anderen juist alleen zoet. Daarnaast drijft het dichtheidsverschil tussen zoet en zout water ook stromingen aan. 
...
De waterstanden in de Waddenzee worden voor een groot deel bepaald door het astronomisch getij. Daarnaast spelen ook wind (zie hoofdstuk \@ref(wind)) en luchtdrukverschillen een rol. Het astronomisch getij ontstaat door de aantrekkingskracht van de maan en de zon, in combinatie met de draaiing van de aarde en de relatieve stand van de maan en zon ten opzichte van elkaar. Wiskundig gezien kan het getij worden ontleed in groot aantal sinusvormige golven met verschillende amplitudes en frequenties: de getijcomponenten. Doordat deze een verschillende frequentie hebben ontstaan fluctuaties op verschillende tijdschalen. 

- De M~2~-component (het dagelijks getij) ontstaat door de aantrekkingskracht van de maan en heeft in de Noordzee en de Waddenzee van alle getijcomponenten de grootste amplitude. 
- De S~2~-component ontstaat door de aantrekkingskracht van de zon en door interactie met M~2~ ontstaat de springtij-doodtij cyclus met een periode van ongeveer 14 dagen. 
- Andere belangrijke periodieke variaties in het getij zijn de zogeheten dagelijkse ongelijkheid (verschil tussen twee hoog- of laagwaters op een dag) en de 18,6-jarige cyclus, als gevolg van de hoek die de maan maakt met het equatorvlak van de aarde. 

Vanaf de zeegaten dringt het getij de Waddenzeebekkens binnen. De geringer wordende diepte en de geometrie van het bekken vervormen het getij. Het getij wordt daardoor asymmetrisch, wat weer belangrijk is voor het residueel transport van zand en slib. De asymmetrie wordt vaak uitgedrukt met de verhouding en het faseverschil tussen de astronomische M~2~-component en de door geometrie bepaalde M~4~-component.
...
df.extrema.jaar <- df.extrema %>%
  ...
  mutate(
    ...
    ) # %T>%                                  # run only when data are updated
  # write_csv(pad)
...
#! sommige locaties hebben geen coordinaten. Dit moet gecheckt worden 

gemGetijslag <- df.extrema.jaar %>% ungroup() %>%
  # filter(jaar == endyear) %>%
  filter(jaar == max(df.extrema.jaar$jaar)) %>%
  pivot_wider(id_cols = c(locatie.naam, jaar), names_from = HL, values_from = jaargemiddelde) %>%
  mutate(`getijslag in cm` = H-L) %>% 
  ungroup() %>%
  group_by(locatie.naam) %>% 
...
De zoetwaterafvoer leidt tot saliniteits- en dichtheidsverschillen tussen het zoute zeewater en het zoetwater, die invloed hebben op de stroming en daarmee het sedimenttransport. Ook heeft de saliniteit (opgenomen in hoofdstuk \@ref(Waterkwaliteit)) van het Waddenwater direct invloed op de ecologie, omdat sommige soorten alleen zoutwater tolereren en anderen juist alleen zoetwater. Het spuien gebeurt zowel via grote als kleine spuisluizen. Het spuien gebeurt zowel via grote als kleine spuisluizen. De drie grootste afvoerpunten zitten in de Afsluitdijk (Den Oever en Kornwerderzand) en bij het Lauwersmeer. De data van kleinere afvoeren wordt minder goed bijgehouden en/of ontsloten, waardoor velen daarvan in deze rapportage buiten beschouwing worden gelaten.
...
hhnkURL = "https://datahuiswadden.openearth.nl/geoserver/zoetwaterafvoeren/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=zoetwaterafvoeren%3Ahhnk_daggemiddelden&outputFormat=csv"
...
hunzeURL <- "https://datahuiswadden.openearth.nl/geoserver/zoetwaterafvoeren/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=zoetwaterafvoeren%3Ahunzeenaas_daggemiddelden&outputFormat=csv"
...
Hoogheemraadschap Hollands Noorderkwartier is momenteel bezig met een project om de debieten van de uitwisselpunten op orde te krijgen. Tijdens het opvragen van de data was dit proces nog in gang, hierdoor is de data die in de onderstaande grafieken wordt gepresenteerd nog niet gevalideerd. 

```{r hhnkAfvoerTijdserie, fig.cap="Tijdseries van zoetwaterafvoeren Hoogheemraadschap Hollands Noorderkwartier. "}
#Er klopt iets nog niet met de daggemiddelden die in de database staan. Nathalie checkt in de database
#
# wat doen met onbetrouwbare waarden? voor nu alleen een filter op betrouwbaar

hhnkDat2<-hhnkDat %>% 
  # make daily average
  dplyr::mutate(datum = as.POSIXct(datum, format="%Y-%m-%d")) %>%
  dplyr::mutate(
...
ggplot() +
  ...
  geom_point(data = hhnkDat2 %>%
               group_by(locatie.naam, jaar, maand) %>% 
               summarize(maandgemiddelde = mean(daggemiddelde), n = n()),
             aes(as.Date(lubridate::ymd(paste(jaar, maand, "15"))), maandgemiddelde, color = "maandgemiddelde"), size = 1
  )  +
  geom_line(data = hhnkDat2 %>%
              ...
              summarize(jaargemiddelde = mean(maandgemiddelde), `n/jaar` = sum(n)) %>% 
              ungroup() %>% 
              filter(`n/jaar` > 10),
            aes(as.Date(lubridate::ymd(paste(jaar, "07", "01"))), jaargemiddelde, color = "jaargemiddelde"), size = 1.5, alpha = 0.8
  ) +
...
Hieronder zijn drie kleine debietlocaties van Wetterskip Friesland aangegeven. Hiervan is Harlingen een spuisluis. Hier worden per kwartier de gemiddelde waterstanden gemeten, zowel voor en achter de sluis en daar wordt het samen met de deur- of rinketstand het debiet berekend. Dit is een ijking door het Waterloopkundig laboratorium (De Voorst) van begin jaren 90.

Ropta en Miedema zijn gemalen. Hier wordt debiet wordt berekend door draaiuren maal een vaste cappaciteit van de pomp. Dit is verder niet gekalibreerd of geijkt.
...
Deze data is tot stand gekomen door bewerkingen en berekingen met formules. Met debietformules worden berekeningen uitgevoerd die de werkelijkheid versimpelen; dit gebeurt nooit helemaal correct; er is dus altijd een zekere foutmargen. Er zijn onzekerheden bij enkele gebruikte debiet-formules waar vanuit de bewerkingen zijn uitgevoerd (met name Spijksterpompen en De Drie Delfzijlen). De Spijksterpompen zijn in 2019 en 2020 vernieuwd, in die tijd waren er noodpompen actief waardoor het debiet in die tijdsperiode onbekend was. Deze debieten zijn dus niet meegenomen in de volume berekeningen. De Drie Delfzijlen bestaat uit drie pompen en een schuif. Deze schuif is vernieuwd in 2019 waardoor er nog geen goede debiet formules beschikbaar zijn. Hierdoor zit er extra onzekerheid in de waardes van de Drie Delfzijlen na 2019. 
...
De afvoeren van Waterschap Hunze en Aas zijn onbewerkte, ongevalideerde gegevens. Hierdoor kunnen er mogelijk wat uitschieters of onregelmatigheden in de data voorkomen. Hierdoor verschillende de hoeveelheden die bij elk van de afvoeren horen. Rozema (max 45 m3/s) en Duurswold (max 28 m3/s) zijn grote gemalen waar Breebaart (max 0,5833 m3/s) een kleine afvoer is. De maximale capaciteiten variëren afhankelijk van de opvoerhoogte. In de grafieken die hieronder gepresenteerd zijn, zitten een aantal meetfouten. Hier wordt nog aan gewerkt bij waterschap Hunze en Aa's.
...
ggplot() +
  geom_point(aes(datum, daggemiddelde, color = "daggemiddelde"), alpha = 1, size = 0.65) +
  geom_point(data = . %>%
               group_by(locatie.naam, jaar, maand) %>% 
               summarize(maandgemiddelde = mean(daggemiddelde), n = n()) %>%
               mutate(datum = lubridate::ymd(paste(jaar, maand, "15"))),
             aes(datum, maandgemiddelde, color = "maandgemiddelde"), 
             size = 1
  )  +
  geom_line(data = . %>%
              group_by(locatie.naam, jaar, maand) %>%
              ...
              ungroup() %>%
              filter(`n/jaar` > 10) %>%
              mutate(datum = as.Date(paste(jaar, "07", "01"), "%Y-%m-%d")),
            aes(as.Date(lubridate::ymd(paste(jaar, "07", "01"))), jaargemiddelde, color = "jaargemiddelde"), size = 1.5, alpha = 0.8
  ) +
  coord_cartesian(ylim = c(NA,NA)) +
  ylab(parlabel) + xlab(xlabel) +
