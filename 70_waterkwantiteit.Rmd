...
```{r getijVanRijn1994, out.width="80%", fig.cap= "Waterbeweging in de tijd (bron: Van Rijn, 1994)" }
...
```{r Laagwaters, fig.height=20, fig.width=10, fig.cap="Jaargemiddeld laagwater (GLW) per stations. De lijn is een linaire regressie door de alle data rekening houdend met een cyclus van 18.6 jaar."}
...
```{r getijslag, fig.height=16, fig.width=10, fig.cap="Jaargemiddelde getijslag per station. De lijn is een linaire regressie door alle data rekening houdend met een cyclus van 18.6 jaar."}
...
gemGetijslag <- rbindlist(map(extrema, function(x) x$HL), idcol = "locatie.naam") %>%
    # filter(HL == "L") %>%
    mutate(jaar = year(time)) %>% 
    filter(jaar != 2021 & jaar > 1970) %>%
  group_by(locatie.naam, jaar, HL) %>% summarise(mean = mean(h)) %>%
