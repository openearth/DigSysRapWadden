...
hollandsnoorderkwartierpath <- "https://datahuiswadden.openearth.nl/geoserver/zoetwaterafvoeren/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=zoetwaterafvoeren%3Ahhnk&outputFormat=csv"
hhnkDat <- data.table::fread(hollandsnoorderkwartierpath, na.strings = "-999")

## issues
## punten en comma's door elkaar als decimal separator. daarom worden het characters
## 1e6 regels gedownloadt. Geoserver geeft niet meer regels. 
...
hunzeenaasPath <- "https://datahuiswadden.openearth.nl/geoserver/zoetwaterafvoeren/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=zoetwaterafvoeren%3Ahunzeenaas&outputFormat=csv"
hunzeDat <- data.table::fread(hunzeenaasPath)

## issues: 1e6 regels downloadt. er zijn er vast meer. 
...
# wat doen met onbetrouwbare waarden? 
...
hhnkDat %>%
  ...
  group_by(locatie.naam, dag, grootheid.code, eenheid.code) %>%
  summarize(daggemiddelde = mean(numeriekewaarde, na.rm = F), n = n()) %>%
  View()
  ggplot() +
  geom_point(color =)
...
# tijdserie plotten
# maandgemiddelden en jaargemiddelden toevoegen

hunzeDat %>%
  sf::st_drop_geometry() %>%
  dplyr::mutate(dag = as_date(datumtijd)) %>%
  dplyr::group_by(dag, locatie.naam, grootheid.code, eenheid.code) %>%
  dplyr::summarise(daggemiddelde = mean(numeriekewaarde, na.rm = TRUE)) %>%
  ggplot2::ggplot(aes(x = dag, y = daggemiddelde)) +
  geom_point() +
    facet_grid(locatie.naam ~., scales = "free_y") +  #scales = "free",
  theme_hy +
  theme(
